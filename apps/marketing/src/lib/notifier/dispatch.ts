...
function buildEmail(args: {
  ...
  const isUS = (args.country || 'DE').toUpperCase() === 'US';
  const severityWord = isUS
    ? args.severity === 'high' ? 'HIGH priority' : args.severity === 'medium' ? 'medium priority' : 'low priority'
    : args.severity === 'high' ? 'HOHE Priorität' : args.severity === 'medium' ? 'mittlere Priorität' : 'geringe Priorität';
  const publishDate = args.filing_publish_date.toLocaleDateString(isUS ? 'en-US' : 'de-DE', {
  ...
  const text = (isUS ? [
    'Hello,',
    '',
    'A regulatory change has been found that may affect one of your Conditions of Approval.',
    '',
    `Project:       ${args.project_name}${args.bauantrag_nr ? ' (' + args.bauantrag_nr + ')' : ''}`,
    `Condition:     ${args.pierced_auflage}`,
    `Priority:      ${severityWord}`,
    `Match:         "${args.matched_keyword}"`,
    '',
    `Source:        ${args.filing_title}`,
    `Published:     ${publishDate}`,
    `Original URL:  ${args.filing_url}`,
    '',
    'Context (matched excerpt):',
    `  …${args.matched_excerpt}…`,
    '',
    'What to do next:',
    `1. Review the original source: ${args.filing_url}`,
    '2. Confirm with your architect whether the change actually affects your Condition',
    '3. If a true condition-piercing event: file a Notice of Construction Delay / change-order with the GC',
    '',
    'This draft was generated by PermitWatchDog — review, adapt, send.',
    '— PermitWatchDog',
...
export async function dispatchAlerts(): Promise<DispatchSummary> {
  ...
  for (const alert of pending) {
    ...
    const { subject, text, html } = buildEmail({
      ...
      country: alert.project.country ?? 'DE',
    });
